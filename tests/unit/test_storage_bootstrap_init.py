...
def test_init_storage_reports_existing_matching_table(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "databricks_tool_agent_template.storage.bootstrap.catalog_exists",
        lambda spark_obj, name: True,
    )
    monkeypatch.setattr(
        "databricks_tool_agent_template.storage.bootstrap.schema_exists",
        lambda spark_obj, target_obj: True,
    )
    monkeypatch.setattr(
        "databricks_tool_agent_template.storage.bootstrap.table_exists",
        lambda spark_obj, target_obj: True,
    )
    monkeypatch.setattr(
        "databricks_tool_agent_template.storage.bootstrap.compare_table_schema",
