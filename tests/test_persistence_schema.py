...
def test_arrow_field_to_spark_sql_type_maps_supported_demo_types() -> None:
    assert (
        schema.arrow_field_to_spark_sql_type(pa.field("run_key", pa.string())) == "STRING"
    )
    assert (
        schema.arrow_field_to_spark_sql_type(pa.field("payload_json", pa.large_string()))
        == "STRING"
    )
    assert (
        schema.arrow_field_to_spark_sql_type(pa.field("event_index", pa.int64())) == "BIGINT"
    )
